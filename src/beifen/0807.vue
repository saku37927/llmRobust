...
# 创建预索引的数据结构以优化性能
# 格式: {(prompt_id, corpus_id): {accuracy: value, attack_results: {method: value}}}
results_index = {}
for res in final_results:
    key = (res['prompt_id'], res['corpus_id'])
    attack_results = {}
    for method in ['charswap', 'deletion', 'easydata', 'embedding', 'swap', 'synonym', 'wordnet']:
        field_name = f'prompt_{method}_result'
        if field_name in res:
            attack_results[method] = res[field_name]
    
    results_index[key] = {
        'accuracy': res['accuracy'],
        'attack_results': attack_results
    }
print(f"预索引创建完成，包含 {len(results_index)} 条记录")
    ...
    if selected_attack_methods and len(selected_attack_methods) > 0:
            ...
            for cid in corpus_ids:
                ...
                if key in results_index:
                    ...
                    for method in selected_attack_methods:
                        if method in results_index[key]['attack_results']:
                            attack_results.append(results_index[key]['attack_results'][method])
        ...
        for pid in prompt_ids:
            for cid in corpus_ids:
                key = (pid, cid)
                if key in results_index:
                    result_map[key] = results_index[key]['accuracy']
        ...
        if selected_attack_methods and len(selected_attack_methods) > 0:
            ...
            for pid in ids:
                for cid in corpus_ids:
                    key = (pid, cid)
                    if key in results_index:
                        robustness_map.setdefault(pid, []).append(results_index[key]['accuracy'])
        ...
        if selected_attack_methods and len(selected_attack_methods) > 0:
            ...
            for cid in ids:
                for pid in prompt_ids:
                    key = (pid, cid)
                    if key in results_index:
                        robustness_map.setdefault(cid, []).append(results_index[key]['accuracy'])
